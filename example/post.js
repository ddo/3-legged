...
var opt = threelegged.post('https://api.twitter.com/1.1/statuses/update.json', token, {
    status: 'three-legged testing'
});
...
request(opt, function(err, res, body) {
    ...
    //delete that status
    request(threelegged.post('https://api.twitter.com/1.1/statuses/destroy/'+body.id_str+'.json', token), function(err, res, body) {
        console.log(err);
    });
